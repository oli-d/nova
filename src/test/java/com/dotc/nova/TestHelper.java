...
public class TestHelper {
	...
	public static <T> T getCaptorValue(ArgumentCaptor<T> capture) {
		return getCaptorValue(capture, MAX_NUM_MS_TO_WAIT_FOR_CAPTURE_VALUE, DELAY_IN_MS_TO_WAIT_FOR_NEXT_CAPTURE_VALUE_CHECK);
	}

	public static <T> T getCaptorValue(ArgumentCaptor<T> capture, long maxMillisToWait, long delayInMs) {
		T returnValue = null;
		long endTime = System.currentTimeMillis() + maxMillisToWait;
		while (returnValue == null && System.currentTimeMillis() <= endTime) {
			try {
