...
public class SpringWiringTest {
    ...
    @Test
    public void eventHandlersProperlyRegistered() {
        applicationContextRunner
            .run(ctx -> {
                MyAnnotatedBean sut = ctx.getBean(MyAnnotatedBean.class);
                EventBus eventBus = ctx.getBean("NOVA.INSTANCE", Nova.class).eventBus;
    ...
    public void eventHandlersProperlyReceiveEventContextIfSignatureOk() throws Exception {
        applicationContextRunner
                .run(ctx -> {
                    MyAnnotatedBean sut = ctx.getBean(MyAnnotatedBean.class);
                    Nova nova = ctx.getBean("NOVA.INSTANCE", Nova.class);
                    EventBus eventEmitter = nova.eventBus;
                    ...
                    assertThat(sut.event3_1Context.eventBus, Matchers.sameInstance(nova.eventBus));
                    assertThat(sut.event3_1Context.metrics, Matchers.sameInstance(nova.metrics));
